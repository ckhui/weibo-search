...
class SearchSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        print("All Date, page", page_count, " >> Count", self.accum_count)
        ...
        if is_empty:
            ...
            while start_date <= end_date:
                ...
                print("Day" , start_str, "to", end_str)
                yield scrapy.Request(url=url,
    ...
    def parse_by_day(self, response):
        ...
        print("All Day", date, " >> Count", self.accum_count)
    ...
    def parse_by_hour(self, response):
        ...
        print("All Hour", start_time)

        if is_empty:
            print('当前页面搜索结果为空')
        elif page_count < 50:
            self.total_page += page_count
            print("All Hour", "Done", "page:", page_count)
            # 解析当前页面
            for weibo in self.parse_weibo(response):
                self.check_environment()
                yield weibo
            ...
            print("All Hour", "Split", "36 regions")
    ...
    def parse_by_hour_province(self, response):
        ...
        print("All Region", province)

        if is_empty:
            print('当前页面搜索结果为空')
        elif page_count < 50:
            self.total_page += page_count
            print("All Region", "Done", "page:", page_count)
            # 解析当前页面
            for weibo in self.parse_weibo(response):
                self.check_environment()
                yield weibo
            ...
            print("All Region", "Split", len(province['city']), "city")
    ...
    def parse_page(self, response):
        self.current_page += 1
        print("crawl page",">>", self.current_page, "/", self.total_page,">>", self.accum_count)
